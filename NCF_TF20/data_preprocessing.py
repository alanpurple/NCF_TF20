...
def _filter_index_sort(raw_rating_path, cache_path):
  ...
  Returns:
    ...
    item IDs.
  """
  valid_cache = tf.compat.v1.gfile.Exists(cache_path)
  if valid_cache:
    with tf.compat.v1.gfile.Open(cache_path, "rb") as f:
    ...
    if not valid_cache:
      tf.compat.v1.logging.info("Removing stale raw data cache file.")
      tf.compat.v1.gfile.Remove(cache_path)

  if valid_cache:
    data = cached_data
  else:
    with tf.compat.v1.gfile.Open(raw_rating_path) as f:
      df = pd.read_csv(f)
    ...
    tf.compat.v1.logging.info("Generating user_map and item_map...")
    ...
    tf.compat.v1.logging.info("Sorting by user, timestamp...")
    ...
    tf.compat.v1.logging.info("Writing raw data cache.")
    with tf.compat.v1.gfile.Open(cache_path, "wb") as f:
...
def instantiate_pipeline(dataset, data_dir, params, constructor_type=None,
  ...
  """
  tf.compat.v1.logging.info("Beginning data preprocessing.")
  ...
  tf.compat.v1.logging.info("Data preprocessing complete. Time: {:.1f} sec."
                  .format(run_time))
